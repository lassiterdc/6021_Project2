...
NO27.ts <- ts(dailyAQ[,2])

# create time series of NO2 without final week
dailyAQ.7 <- dailyAQ[1:384,]
NO2.ts <- ts(dailyAQ.7[,2])
```
...
max.omega.no27<-pg.NO2$freq[which(pg.NO2$spec==max(pg.NO2$spec))]

max.omega.no27

# Looking at other peaks that we could try and consider in the model.
...
```
We examined the 10 most significant peaks from the periodogram and stored them in sorted.Ts. We can use these in building a model by going through and seeing if adding the next period would decrease the AIC.
  
## Hypotheses from exploratory data analysis:

There is an increasing trend in NO2 emissions. This could be due to increasing cars on the road over time. We also think that other components such as weekly, monthly, and yearly trends could play a part. This could be a function of the number of cars on the road, and perhaps traffic is higher on the week days or weekends.
...
This indicates that the residuals could be modeled as either AR(2) or as an ARMA process.


```{r, fig.height=11, fig.width=8, fig.align='center'}
 plt_resid_ACF <- data_resids %>%
  dplyr::filter(.model %in% c("trnd_ssn_wkly_mnthly", "trnd_ssn_complex_trig", "trnd_ssn_complex_trig_and_dummy")) %>% ACF(.resid) %>% autoplot() + ggtitle("Trig Functions: NO2 Residuals ACF plot") + my_theme

plt_resid_PACF <- data_resids %>%
  dplyr::filter(.model %in% c("trnd_ssn_wkly_mnthly" ,"trnd_ssn_complex_trig", "trnd_ssn_complex_trig_and_dummy")) %>% PACF(.resid) %>% autoplot() + ggtitle("Trig Functions: NO2 Residuals PACF plot") + my_theme
...
# Different simulation here (change)
```{r}
# Next, look at periodogram
ps <- spec.pgram(dailyAQ_train,spans=9,demean=T,log='no')
...
fmodel <- model(trend = TSLM(NO2 ~ trend()),
        trnd_ssn_wkly = TSLM(NO2 ~ trend() + sin(2*pi*step/7) + cos(2*pi*step/7)))
fmodel %>% dplyr::select(trend) %>% report()
sim <- as.numeric(dailyAQ_train)
...
# Means
a <- mean(dailyAQ_train$NO2)
b <- mean(NO2.ts)
abs(b-a)/b     #0

# Variances
c <- var(dailyAQ_train$NO2)
d <- var(NO2.ts)
abs(d-c)/d     #0
