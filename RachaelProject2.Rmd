...
# now we know concentration is significant and it is positive
# Plot no27.trend.seasonal model
ggplot(dailyAQ.7, aes(x=Group.1,y=NO2.GT.)) + geom_line() + 
  geom_line(aes(x=Group.1,y=no27.trend.seasonal$fitted.values),color="red") 

# Model diagnostics for no27.trend.seasonal
...
# that we need to capture as well
...
no27.auto.forecast <- forecast(no27.auto, h=6)

plot(no27.auto.forecast)
...
E_Y.pred <- predict(no27.trend.seasonal, newdata=dailyAQ)
e_t.pred <- forecast(no27.auto, h=6)
next.wk.prediction <- E_Y.pred + e_t.pred$mean

# MSE:

mean((next.wk.prediction- dailyAQ$NO2.GT)^2)
#__, can repeat w alternate models

# Plot actual values and predicted values
plot(ts(dailyAQ$NO2.GT),type='o',ylim=c(25,60))
